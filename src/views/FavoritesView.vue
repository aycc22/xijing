...
import { loadFavoriteList, type LearningListItem } from '../lib/userLearning'
...
async function load() {
  ...
  try {
    items.value = await loadFavoriteList(supabase, auth.user.value.id)
  } catch (err) {
    error.value = formatErrorMessage(err, '加载收藏失败')
  }
  loading.value = false
}
  ...
  <div>
    <section class="flex flex-wrap items-end justify-between gap-3 py-4 md:py-6">
      <div>
        ...
        <p class="page-lede">收藏的题目会集中在这里，点击即可回到对应题库练习。</p>
      </div>
    ...
    <div v-else-if="!items.length" class="surface py-14 text-center">
      <p class="m-0 font-medium text-ink">还没有收藏</p>
      <p class="mt-1.5 text-sm text-muted">刷题时可以对题目点收藏。</p>
